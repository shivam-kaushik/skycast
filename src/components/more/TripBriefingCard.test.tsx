...
describe('TripBriefingCard', () => {
  it('renders destination form in initial state', async () => {
    let tree!: renderer.ReactTestRenderer
    await act(async () => {
      tree = renderer.create(<TripBriefingCard weatherCtx={CTX} />)
    })
    const json = JSON.stringify(tree.toJSON())
    expect(json).toMatch(/Trip Weather Briefing/)
    expect(json).toMatch(/Generate Briefing/)
    expect(json).toMatch(/Destination/)
  })
})
